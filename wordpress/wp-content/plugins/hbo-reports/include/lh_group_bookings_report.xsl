...
<xsl:template match="view">
    ...
    <div class="d-flex mt-4">
        <span class="bi-book" style="margin: 5px 10px 5px 100px;"/>
        <h2>Annoying Groups Who Really Should Book the Whole Room</h2>
    </div>

    <div class="card text-center">
        <div class="card-body">
            <xsl:choose>
                <xsl:when test="mostly_full_record">
                    <xsl:call-template name="mostly_full_report_data"/>
                </xsl:when>
                <xsl:otherwise>
                    <div class="ml-5 mb-2 mt-2 font-italic">
                        <h6>No data available.</h6>
...
<xsl:template name="mostly_full_report_data">
    <xsl:if test="../property_manager != 'cloudbeds'">
        <p style="padding-left: 20px;"><strong>BOLD</strong> entries have not been viewed before in Little Hotelier.</p>
    </xsl:if>

    <table id="mostly_full_dorms_rpt" class="table table-striped table-hover">
        <thead class="thead-dark">
            <tr>
                <th scope="col">Guest Name(s)</th>
                <th scope="col">Booking Reference</th>
                ...
                <th scope="col">Booked Date</th>
                <th scope="col">Number of<br/>Guests</th>
                <th scope="col">Room<br/>Capacity</th>
            </tr>
        </thead>
        <tbody>
            <xsl:apply-templates select="mostly_full_record"/>
        </tbody>
    </table>

<script type="text/javascript">
  jQuery('#mostly_full_dorms_rpt').DataTable({
    "paging": false,
    "searching": false,
    "order": [[3, 'asc']]
  });
...
<xsl:template match="mostly_full_record">
    <tr data-toggle="tooltip" data-html="true">
        <xsl:attribute name="class">
            <xsl:if test="viewed_yn = 'N'">unread</xsl:if>
        </xsl:attribute>
        ...
        <td class="text-left"><xsl:attribute name="data-order"><xsl:value-of select="booked_datetime"/></xsl:attribute><xsl:value-of select="booked_date"/></td>
        <td><xsl:value-of select="num_guests"/></td>
        <td><xsl:value-of select="room_capacity"/></td>
    </tr>
</xsl:template>
