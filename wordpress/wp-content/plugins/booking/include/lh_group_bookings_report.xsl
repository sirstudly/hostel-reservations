...
#group_bookings_rpt tbody tr:nth-child(odd) td,#split_room_rpt tbody tr:nth-child(odd) th {
...
tr.unread {
    ...
    <div class="wpdevbk center">
        <h3>Bookings with 6 or More Guests</h3>
    </div>
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            <form name="group_bookings_form" method="post" id="group_bookings_form" class="form-inline">
                <input type="hidden" name="allocation_scraper_job" id="allocation_scraper_job" value="I can't wait. Run it now!"/>
                <p style="margin-left:20px"><a class="btn-primary" onclick="javascript:group_bookings_form.submit();">I can't wait. Run it now!</a></p>
                <br/><br/>
            </form>
        </xsl:otherwise>
    </xsl:choose>
